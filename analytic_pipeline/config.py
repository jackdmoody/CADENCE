...
class PairConfig:
    """
    Per-(src_ip, dst_ip) pair grouping and filtering parameters.

    After IForest pre-filtering, the pipeline groups anomalous flows by
    (src_ip, dst_ip) pair. This replaces DBSCAN clustering.

    min_observations (8)
        Minimum number of flows a pair must have to be evaluated.
        Pairs below this cannot support reliable ACF analysis.

    max_pairs (5000)
        Safety cap on the number of pairs to evaluate. Applied after
        sorting by flow count descending so the richest pairs are always
        evaluated. In practice, 30-day windows with 100k anomalies
        typically yield 10k–50k unique pairs; the cap prevents the SAX
        stage from becoming a bottleneck.

    min_pair_flows (3)
        Hard minimum: pairs with fewer flows than this are skipped
        before even computing SAX. Prevents trivially short sequences
        from generating spurious results.
    """
    min_observations: int = 8
    min_pair_flows:   int = 3
    max_pairs:        int = 5_000
...
class CorroborationConfig:
    """Multi-source corroboration parameters for DNS and HTTP validation."""
...
class BDPConfig:
    ...
    def from_json(cls, path: str) -> "BDPConfig":
        ...
        return cls(
            ...
            corroboration=CorroborationConfig(**d.get("corroboration", {})),
        )
