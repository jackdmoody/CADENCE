...
def run_isolation_forest(
    ...
    # Bypass: when the pool is very small, IForest has no meaningful distribution
    # to learn anomaly boundaries from. Pass all pairs through so SAX/periodicity
    # can make the call instead of wasting the only legitimate candidates.
    ...
    if len(pair_df) < MIN_PAIRS_FOR_IFOREST:
        ...
        ).fit(pair_df[features].fillna(0).astype(float))
