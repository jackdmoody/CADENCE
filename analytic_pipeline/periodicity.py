...
def score_pair_periodicity(
    ...
    # Period estimate from binned ACF: bin_size = median_iat/2.
    # ACF lag=1 on a half-period binned series = one full beacon period = bin_size * 2 = median_iat.
    # However the median IAT already equals the beacon period directly, so use it as-is.
    iat_median = float(np.median(iat))
    # lag * bin_size gives half-periods; multiply by 2 to get full period, then divide by 2
    # since bin_size = median/2 already encodes the period. Net: period = acf_lag * iat_median.
    # Cross-check: for lag=1, bin_size=period/2, acf_lag*bin_size*2 = period. Simplified:
...
def score_all_pairs(
    ...
    # Build set of pair_ids that passed SAX (if available)
    sax_pass_ids: Optional[set] = None
    if sax_df is not None and "sax_prescreen_pass" in sax_df.columns:
        sax_pass_ids = set(
            sax_df[sax_df["sax_prescreen_pass"]]["pair_id"].astype(str).tolist()
        )
        log.info("ACF analysis restricted to %d SAX-passing pairs", len(sax_pass_ids))

    results = []

    for (src, dst), pair_df in df_anomalies.groupby(["src_ip", "dst_ip"]):
        pair_id = f"{src}→{dst}"

        # Skip if SAX filter active and this pair didn't pass
        if sax_pass_ids is not None and pair_id not in sax_pass_ids:
            results.append({
                "pair_id":            pair_id,
                "src_ip":             src,
                "dst_ip":             dst,
        ...
        # Skip pairs below minimum observation count
