...
class BDPPipeline:
    ...
    def corroborate(
        ...
        """Cross-validate beacon candidates against DNS and HTTP evidence."""
        log.info("Stage 9: corroborate()")

        if dns_log_path is None and http_log_path is None:
            log.warning("No DNS or HTTP logs provided — skipping corroboration.")
            return pd.DataFrame()
