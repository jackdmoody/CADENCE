...
def apply_prefilter(
    ...
    dst_fanin_threshold: float = DEFAULT_DST_FANIN_THRESHOLD,
    failed_conn_threshold: float = DEFAULT_FAILED_CONN_RATIO,
    custom_allowlist: Optional[set] = None,
    custom_cdn_prefixes: Optional[list] = None,
    ...
    raw_df                : Raw flow-level DataFrame (needed for fanin and
                            ...
                            are skipped.
    dst_fanin_threshold   : Fraction of unique sources above which a
                            destination is considered shared infrastructure.
                            Default: 0.25 (25%).
    failed_conn_threshold : Fraction of failed connection states above which
                            a pair is considered dead. Default: 0.90 (90%).
    ...
    Returns
    -------
    filtered_df : Pairs that pass the pre-filter → continue to IForest.
    removed_df  : Pairs removed by the pre-filter → logged for audit.
    """
    n_before = len(pair_df)

    # --- Build lookup sets from raw flow data ---
    if raw_df is not None and not raw_df.empty:
        # Identify column names (handle schema variations)
        src_col = "src_ip" if "src_ip" in raw_df.columns else "source.ip"
        dst_col = "dst_ip" if "dst_ip" in raw_df.columns else "destination.ip"

        # Find conn_state column
        state_col = None
        ...
        high_fanin_dsts = _build_high_fanin_set(
            raw_df, dst_fanin_threshold, src_col, dst_col,
        )

        if state_col:
            dead_pairs = _build_dead_pair_set(
                raw_df, failed_conn_threshold, src_col, dst_col, state_col,
            )
        else:
