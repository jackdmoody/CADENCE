...
def screen_pairs(
    ...
    if len(pair_counts) > pc.max_pairs:
        log.info(
            "screen_pairs(): capping at %d pairs (of %d total unique pairs)",
            pc.max_pairs, len(pair_counts),
        )
        pair_counts = pair_counts.head(pc.max_pairs)

    log.info("screen_pairs(): evaluating %d (src,dst) pairs", len(pair_counts))
    ...
    for _, row in pair_counts.iterrows():
        ...
        log.debug(
            "Pair %s→%s  SAX: passes=%s  n=%d",
            src, dst, result["sax_passes"], result["n_observations"],
        )
