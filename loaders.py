...
def _rename_and_subset(df: pd.DataFrame, keep_cols: tuple) -> pd.DataFrame:
    ...
    df = df.rename(columns={
        ...
        "source.bytes":             "orig_bytes",
        "destination.bytes":        "resp_bytes",
        "source.ip_bytes":          "orig_ip_bytes",
        "destination.ip_bytes":     "resp_ip_bytes",
...
def load_and_prepare(cfg: BDPConfig) -> pd.DataFrame:
    ...
    # --- 2. Schema normalisation (must run before any column references) ---
    # Normalise timestamp column name: SQL path returns 'timestamp';
    # CSV exports may arrive as 'ts' or '_timestamp'.
    for ts_candidate in ["ts", "_timestamp"]:
        if ts_candidate in df.columns and "timestamp" not in df.columns:
            df = df.rename(columns={ts_candidate: "timestamp"})
            break
    df = _rename_and_subset(df, cfg.features.keep_cols)

    # --- 3. Parse timestamps ---
    # Try Unix seconds first, fall back to milliseconds, then ISO strings.
    ...
    log.info("Unique calendar days in extract: %d", df["datetime"].dt.date.nunique())

    # --- 4. Type coercion ---
    df = _convert_object_columns(df)

    # --- 5. Periodic time encoding ---
    # Keep timestamp as numeric Unix seconds for IAT/ACF calculations.
    # The datetime column is used for display/filtering; overwriting timestamp
    # with a datetime causes silent failures in periodicity.py float conversions.
    df = _encode_time_periodically(df, timestamp_col="datetime")

    # --- 6. Derived features ---
    df["total_bytes"] = df[["resp_bytes", "orig_bytes"]].fillna(0).sum(axis=1)
